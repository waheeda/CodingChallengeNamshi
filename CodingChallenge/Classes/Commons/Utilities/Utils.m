...
{
   NetworkStatus status =   [ (AppDelegate *)[[UIApplication sharedApplication] delegate] currentReachabilityStatus];
...
+ (NSString *) getDuration : (NSDate *) startDate EndDate : (NSDate *) endDate {
	...
	return [Utils formatMinutes:(int)[conversionInfo minute]];
}
...
+ (int) getBatteryLevel {
	...
	return (int)[percent integerValue];
}
